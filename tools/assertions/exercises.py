...
@allure.step("Check exersise schema")
def assert_exercise(actual: ExerciseSchema, expected: ExerciseSchema):
    ...
    :raises AssertionError: Если хотя бы одно поле не совпадает.
    """
    logger.info("Check exersise schema")
...
def assert_update_exercise_response(update_exercise_response: UpdateExerciseResponseSchema,
                                    update_exercise_request: UpdateExerciseRequestSchema):
    """
    Проверяет,  что ответ на обновление задания соответствуют отправленным данным.

    :param update_exercise_response: Фактические данные задания.
    :param update_exercise_request: Ожидаемые данные задания.
    :raises AssertionError: Если хотя бы одно поле не совпадает.
    """
    ...
    assert_equal(update_exercise_response.exercise.estimated_time,
                 update_exercise_request.estimated_time, "estimated_time")


@allure.step("Check exercise not found error")
def assert_exercise_not_found_response(actual: InternalErrorResponseSchema):
    """
    Проверяет текст ошибки, если упражнение не найдено.

    :param actual: Фактический ответ.
    :raises AssertionError: Если фактический ответ не соответствует ошибке "Exercise not found"
    """
    logger.info("Check exercise not found error")
